...
void describe('lib/outfit-layout', () => {
  void it('normalizes invalid layouts to an empty array', () => {
    assert.deepEqual(normalizeLayout(null), []);
    assert.deepEqual(normalizeLayout({}), []);
    assert.deepEqual(normalizeLayout('bad'), []);
  });

  void it('retains valid rows and entries', () => {
    const layout = [
  ...
  void it('drops malformed rows and entries during normalization', () => {
    const layout = [
  ...
  void it('collects unique item ids from a valid layout', () => {
